# `scripts/interferometer/searches`

Single-**search** runs on interferometer data: one sampler, one model, one seed. Where `slam/` asks
...
```
scripts/interferometer/searches/<sampler>/<target>.py
```
